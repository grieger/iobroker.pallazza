    ...
    Object.keys(state).forEach(function (key) {
      if (typeof state[key] === "object" && !Array.isArray(state[key])) {
        ...
        adapter.getObject(stateName, function (err, object) {
          if (err) {
            adapter.log.error(`Error getting object for ${stateName}: ${err}`);
            disableAdapter = true;
          }
          ...
          if (object !== null) {
            adapter.getState(stateName, function (err, state) {
              if (err) {
                adapter.log.error(`Error getting state for ${stateName}: ${err}`);
                disableAdapter = true;
              }
              ...
              if (state !== null) {
                ...
                if (state.val !== newValue) {
                  adapter.log.debug(`State changed for ${stateName}: ${newValue} (was: ${state.val})`);
                  adapter.setState(stateName, newValue, true);
                }
              } else {
                adapter.log.debug(`Initial setting of state ${stateName} to ${newState.value}`);
                if (typeof newState.value === "object") {
            ...
            adapter.log.warn(`State ${stateName} does not exist. Please check the data model.`);
            missingState = true;
          }
